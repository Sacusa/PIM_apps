...
int main(int argc, char ** argv) {
    ...
    // Create two streams, one for PIM kernel and one for non-PIM kernel
    cudaStream_t streams[2];
    cudaStreamCreate(&streams[0]);
    cudaStreamCreate(&streams[1]);

    // Instantiate and launch PIM kernel
    pim_state_t *pim_state = init_pim(STREAM_ADD, 1048576, 1);
    launch_pim(pim_state, streams[1]);

	// Get GICOV matrices corresponding to image gradients
	long long GICOV_start_time = get_time();
	MAT *gicov = GICOV(grad_x, grad_y, streams[0]);
	long long GICOV_end_time = get_time();

	// Dilate the GICOV matrices
	long long dilate_start_time = get_time();
	MAT *img_dilated = dilate(gicov, streams[0]);
	...
	int num_snaxels = 20;
	ellipsetrack(cell_file, QAX_CENTERS, QAY_CENTERS, k_count, radius, num_snaxels, num_frames, streams[0]);
	printf("           Total: %.5f seconds\n", ((float) (get_time() - tracking_start_time)) / (float) (1000*1000*num_frames));

    // Synchronize the kernels and free PIM memory
