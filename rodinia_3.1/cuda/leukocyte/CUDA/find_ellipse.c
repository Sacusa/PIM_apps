...
MAT *GICOV(MAT *grad_x, MAT *grad_y, cudaStream_t stream) {
...
MAT *dilate(MAT *img_in, cudaStream_t stream) {
	// Determine the dimensions of the frame
	...
	float *host_img_dilated = dilate_CUDA(max_gicov_m, max_gicov_n, strel_m, strel_n, stream);
