...
// Constants used in the , cudaStream_t streamMGVF computation
...
void IMGVF_cuda(MAT **I, MAT **IMGVF, double vx, double vy, double e, int max_iterations, double cutoff, int num_cells, cudaStream_t stream) {

	// Initialize the data on the GPU
	IMGVF_cuda_init(I, num_cells, stream);

	// Compute the MGVF on the GPU
	IMGVF_kernel <<< num_cells, threads_per_block, 0, stream >>>
	...
	IMGVF_cuda_cleanup(IMGVF, num_cells, stream);
}


// Initializes data on the GPU for the MGVF kernel
void IMGVF_cuda_init(MAT **IE, int num_cells, cudaStream_t stream) {
	...
	cudaMemcpyAsync(device_I_all, host_I_all, total_mem_size, cudaMemcpyHostToDevice, stream);
	cudaMemcpyAsync(device_IMGVF_all, host_I_all, total_mem_size, cudaMemcpyHostToDevice, stream);
    cudaStreamSynchronize(stream);

	// Copy pointer arrays to device
	cudaMemcpyAsync(device_I_array, host_I_array, num_cells * sizeof(float *), cudaMemcpyHostToDevice, stream);
	cudaMemcpyAsync(device_IMGVF_array, host_IMGVF_array, num_cells * sizeof(float *), cudaMemcpyHostToDevice, stream);
    cudaStreamSynchronize(stream);

	// Copy memory dimension arrays to device
	cudaMemcpyAsync(device_m_array, host_m_array, num_cells * sizeof(int), cudaMemcpyHostToDevice, stream);
	cudaMemcpyAsync(device_n_array, host_n_array, num_cells * sizeof(int), cudaMemcpyHostToDevice, stream);
    cudaStreamSynchronize(stream);
}


// Copies the results of the MGVF kernel back to the host
void IMGVF_cuda_cleanup(MAT **IMGVF_out_array, int num_cells, cudaStream_t stream) {
	// Copy the result matrices from the device to the host
	cudaMemcpyAsync(host_I_all, host_IMGVF_array[0], total_mem_size, cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
