...
float *GICOV_CUDA(int grad_m, int grad_n, float *host_grad_x, float *host_grad_y, cudaStream_t stream) {
	...
	cudaMemcpyAsync(device_grad_x, host_grad_x, grad_mem_size, cudaMemcpyHostToDevice, stream);
	cudaMemcpyAsync(device_grad_y, host_grad_y, grad_mem_size, cudaMemcpyHostToDevice, stream);
	...
	GICOV_kernel <<< num_blocks, threads_per_block, 0, stream >>> (grad_m, device_gicov);
    cudaStreamSynchronize(stream);
	...
	cudaMemcpyAsync(host_gicov, device_gicov, grad_mem_size, cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
...
float *dilate_CUDA(int max_gicov_m, int max_gicov_n, int strel_m, int strel_n, cudaStream_t stream) {
	...
	dilate_kernel <<< num_blocks, threads_per_block, 0, stream >>> (max_gicov_m, max_gicov_n, strel_m, strel_n, device_img_dilated);
    cudaStreamSynchronize(stream);
	...
	cudaMemcpyAsync(host_img_dilated, device_img_dilated, max_gicov_mem_size, cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
