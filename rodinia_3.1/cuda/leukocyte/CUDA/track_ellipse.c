...
void ellipsetrack(avi_t *video, double *xc0, double *yc0, int Nc, int R, int Np, int Nf, cudaStream_t stream) {
	/*
	...
	*/
	...
	for (frame_num = 1; frame_num <= Nf; frame_num++) {
		...
		MAT **IMGVF = MGVF(IE, 1, 1, Nc, stream);
...
MAT **MGVF(MAT **IE, double vx, double vy, int Nc, cudaStream_t stream) {
	/*
	...
	*/
	...
	IMGVF_cuda(IE, IMGVF, vx, vy, epsilon, iterations, converge, Nc, stream);
