...
float pgain( long x, Points *points, float z, long int *numcenters, int kmax, bool *is_center, int *center_table, bool *switch_membership, bool isCoordChanged,
							double *serial_t, double *cpu_to_gpu_t, double *gpu_to_cpu_t, double *alloc_t, double *kernel_t, double *free_t, cudaStream_t cu_stream)
{	
#ifdef CUDATIME
	float tmp_t;
	...
	*alloc_t += (double) tmp_t;
	...
	if(isCoordChanged || iter == 0)
	{
		CUDA_SAFE_CALL( cudaMemcpyAsync(coord_d,  coord_h,	 num * dim * sizeof(float), cudaMemcpyHostToDevice, cu_stream) );
	}
	CUDA_SAFE_CALL( cudaMemcpyAsync(center_table_d,  center_table,  num * sizeof(int),   cudaMemcpyHostToDevice, cu_stream) );
	CUDA_SAFE_CALL( cudaMemcpyAsync(p,  points->p,				   num * sizeof(Point), cudaMemcpyHostToDevice, cu_stream) );
	
	CUDA_SAFE_CALL( cudaMemsetAsync((void*) switch_membership_d, 0,			num * sizeof(bool), cu_stream)  );
	CUDA_SAFE_CALL( cudaMemsetAsync((void*) work_mem_d,  		0, stride * (nThread + 1) * sizeof(float), cu_stream) );
	...
	*cpu_to_gpu_t += (double) tmp_t;
	...
	kernel_compute_cost<<<grid_size, THREADS_PER_BLOCK, 0, cu_stream>>>(	
															num,					// in:	# of data
	...
	*kernel_t += (double) tmp_t;
	...
	//=======================================
	CUDA_SAFE_CALL( cudaMemcpyAsync(work_mem_h, 		  work_mem_d, 	stride * (nThread + 1) * sizeof(float), cudaMemcpyDeviceToHost, cu_stream) );
	CUDA_SAFE_CALL( cudaMemcpyAsync(switch_membership, switch_membership_d,	 num * sizeof(bool),  cudaMemcpyDeviceToHost, cu_stream) );
