...
void master(fp timeinst,
	...
	cudaMemcpyAsync(d_initvalu, initvalu, d_initvalu_mem, cudaMemcpyHostToDevice, stream);
	cudaMemcpyAsync(d_params, parameter, d_params_mem, cudaMemcpyHostToDevice, stream);
	...
	blocks.x = 2;
	blocks.y = 1;
	kernel<<<blocks, threads, 0, stream>>>(	timeinst,
															d_initvalu,
															d_finavalu,
															d_params,
															d_com);
	...
	cudaMemcpyAsync(finavalu, d_finavalu, d_finavalu_mem, cudaMemcpyDeviceToHost, stream);
	cudaMemcpyAsync(com, d_com, d_com_mem, cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
